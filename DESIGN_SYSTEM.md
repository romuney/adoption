...
**Если вы правите виджеты Proteus Adoption** — сначала раздел 16: это фактический
профиль виджетов борда (шрифт, кегли, веса, отступы, поведение) и список его
отличий от ядра. Для виджетов он главнее разделов 2–4.
...
## 16. Профиль Proteus Adoption: как реализовано в виджетах (2026-09-23)

Разделы 1–15 — общее ядро TeamPulse для автономных HTML-отчётов. Этот раздел —
**фактическая** дизайн-система трёх виджетов листа «Отчёты» борда Proteus Adoption:
шапки (`pa-strip`), каталога (`pa-reports-body`) и панели «Аудитория области»
(`pa-area`). Значения списаны с кода (`Виджеты/*.chart.js`), а не придуманы.
Где профиль расходится с ядром, побеждает профиль: расхождения собраны
в конце раздела (16.8) вместе с причинами. Когда меняется код виджетов,
обновляется и этот раздел.

Среда: каждый виджет — eCharts-чарт `react_sanbbox` в своём iframe Proteus.
Отсюда два ограничения: у каждого виджета свой `:root`-блок токенов (общих стилей
на борд нет), и тултип живёт в `body`, вне корня виджета, поэтому шрифт и цвета
ему задаются явно.
...
- Один стек на весь виджет, **включая тултип в `body`**: наследовать ему не от кого.
- Шрифт не подключается ссылкой: в Proteus берётся Inter, если он установлен, иначе
  системный. Вёрстка обязана выглядеть прилично на обоих вариантах.
- Все числа — `font-variant-numeric: tabular-nums`: в таблицах, KPI, тултипах, пилюлях
  частоты. Разряды — тонким пробелом, минус — типографский «−», компактная запись
  1,2K / 3,4M.

### 16.2. Кегли: восемь ролей

| Роль | px | Где |
|---|---|---|
| micro | 9–9,5 | строка медианы в шапке когорт, значок ⓘ |
| cap | 10,5 | шапки колонок (КАПИТЕЛЬ, `letter-spacing:.3px`), подпись «ВЫБРАННЫЕ ФИЛЬТРЫ» и «СЧИТАТЬ» (КАПИТЕЛЬ, `.5px`), вторая строка ячейки (владелец, логин, «УС-3 · 5 подразделений»), подписи оси X, подвал тултипа |
| note | 11,5 | подзаголовки панелей, подписи KPI, пилюли фильтров, вкладки, кнопки тулбара, дельты, пояснения, текст тултипа |
| control | 12 | сегменты периода и переключатели в шапке |
| body | 12,5 | текст и числа таблиц, «Что видно в данных», значения в тултипе |
| title | 14,5 | заголовок панели («Каталог», «Динамика · весь Proteus») |
| hero | 24 | главное число KPI (`letter-spacing:-.4px`, `line-height:1.15`) |
| SVG | 10–13 | подписи значений у марок 11 (плотная сетка 10); заголовки графиков 13/12; легенда 12; ось 10,5 |

Новых кеглей «на глаз» не заводить: нужное выбирается из таблицы по роли.

### 16.3. Веса: потолок 600

| Вес | Что |
|---|---|
| 400 | данные: числа в ячейках, тело текста, пояснения, сноски, подзаголовки |
| 500 | по чему ведут взглядом: имя строки (первая колонка), шапки колонок, итоговая строка, пилюли, вкладки, подписи KPI, значения в тултипе, заголовок «Что видно в данных» |
| 600 | только то, на что надо смотреть: число KPI, заголовок панели, число корзины частоты, метка начала месяца/года на оси X |
| 700+ | не используется |

Ниже 600 иерархию несут **цвет, кегль и капитель**, а не насыщенность. Второй и
следующие текстовые столбцы таблицы — 400 и `ink2`, даже если это имена.

### 16.4. Цвет текста и акцента

Каждый виджет объявляет в корне одни и те же токены:

```css
--card:#fff; --line:#e7e9ee; --line2:#eef0f3; --bg:#f6f6f6;       /* холст = фон борда Proteus */
--ink:#23272e;  --ink2:#454b55;  --muted:#8a909c;  --muted2:#aab0bb;
--green:#12b048; --green-bg:#bff2cd; --green-tx:#0a8f3c;
--red:#f51f1f;   --red-bg:#ffcccc;   --red-tx:#d11414;
--act:#245FD4; --act-ink:#1B4AA8; --blue-bg:#EAF0FC; --act-line:#C3D4F5;   /* кобальт — основной синий отчёта (2026-09-25) */
```

| Токен | Роль |
|---|---|
| `ink` | заголовки, первая колонка, итоги, число KPI |
| `ink2` | тело таблиц и текста |
| `muted` | подписи, шапки колонок, подзаголовки, вторая строка ячейки |
| `muted2` | третий план: рамки значков, разделители |
| `act` | активное: ссылки, текстовые кнопки, стрелка сортировки, включённый переключатель |
| `act-ink` / `blue-bg` / `act-line` | выбранная строка и «своя» пилюля (текст / фон / рамка) |

- **Выбор людей** (людская шина из «Кто смотрит») — сиреневые пилюли: фон `#f3ecff`,
  рамка `#e2d4ff`, текст `#5a2fc2`. Так он отличается от выбора отчётов (голубой).
- **Серии данных:** новые `#245FD4`, вернувшиеся `#AA77FF`, продолжающие `#7FA3EA`,
  просмотры `#5b6478`.
- **Корзины частоты:** пять ступеней от светлого к тёмному: `#D3E0FA #A9C1F2
  #7FA3EA #4A7BE0 #245FD4` (кобальтовая шкала; 4-ступенчатая — `#D3E0FA #8AAAEC #4A7BE0 #245FD4`).
- **Когорты:** дивергентная шкала «жёлтый `#FFD758` ↔ белый ↔ кобальтовый `#8AAAEC`»,
  три ступени в каждую сторону. Края шкалы — максимальное отклонение **своей
  стороны** (вверх и вниз отдельно), поэтому крайняя ступень легенды всегда
  подсвечивает хотя бы одну ячейку.

### 16.5. Раскладка листа и воздух

Шкала отступов: 2 · 4 · 6 · 8 · 10 · 12 · 14 · 16 px. Исключения названы явно
(`chart-gap` 26, отступ столбика когорт 14, отступ списка «Что видно» 44).

```
┌ Шапка — одна строка, ~64 px ─────────────────────────────────────────────┐
│ [30 дней|20 недель|12 месяцев|8 кварталов] │ СЧИТАТЬ (●Опубл.)(●Актуал.)(●Без влад.)   ● данные за вчера │
└──────────────────────────────────────────────────────────────────────────┘
 ВЫБРАННЫЕ ФИЛЬТРЫ [пилюли…]                (строка 34 px, над панелью — без подписи)
┌ Каталог ~38% ─────────────┐  ┌ KPI ×5 (gap 10) ───────────────────────────┐
│                           │  ├ Что видно в данных (42 px, раскрывается ↓) ┤
│                           │  ├ Карточка вкладок — остаток высоты          ┤
└───────────────────────────┘  └────────────────────────────────────────────┘
```

**Уровень листа**
- Шапка: одна строка, gap 8. Разделитель 1 × 20 px с полями 0 4.
- Строка «Выбранные фильтры»: высота **34**, снизу 8, внутри 0 4. Подпись
  «ВЫБРАННЫЕ ФИЛЬТРЫ» одна на лист — над каталогом. Над панелью строка той же
  высоты, без подписи и подсказки.
- Правая колонка: KPI-сетка 5 × `minmax(0,1fr)`, gap 10. Блок «KPI + Что видно»:
  gap 10, снизу 12. Карточка вкладок забирает остаток высоты, графики «Динамики»
  делят его между собой.

**Карточка / панель**
- Радиус 12, фон `card`, без тени (контраст с холстом `#f6f6f6` достаточен).
- Заголовок: padding 14 16, gap 10. Подзаголовок — вторая строка, отступ 2.
- Тело: padding 14 16. У таблиц — без верха и низа: скролл внутри `-tbl-scroll`,
  шапка таблицы липкая.
- KPI: padding 12 14. Подпись → число 4, число → дельта 6.

**Таблицы** (общий CSS-блок «ТАБЛИЦЫ», дословно одинаковый в каталоге и панели)
- Строка **44 px**, нижняя граница `line2`.
- `th`: padding 9 8, cap, КАПИТЕЛЬ, 500, `muted`. Стрелка сортировки — только у
  активной колонки.
- `td`: padding 6 8, числа вправо. Первая колонка: отступ слева 12, 500, `ink`,
  вторая строка через 2 px (cap, `muted`).
- Итог: 500, `ink`, нижняя граница 2 px `line`. Выбранная строка: фон `blue-bg` +
  полоса 3 px `act` слева (inset).
- Пагинатор: padding 8 10, gap 10, «Показано a–b из N».
- Иконка в ячейке (скрепка ссылки) — **висячий отступ**: иконка слева, текст
  отдельным блоком, перенос уходит под текст, а не под иконку.

**Сводная таблица групп** («Кто смотрит» с группировкой): `table-layout:fixed`, имя —
30% (длинное обрезается «…»), «Доля» — 15%, остальные числовые — поровну. Ячейка доли —
полоса 40 px + число в поле 42 px вправо: полосы всех строк на одной вертикали. В шапке
короткие подписи («Просм.», «Пост.»), полные — в подсказке и выгрузке.

**Анимация (ДС 6.5)** — только в рендере с НОВЫМИ данными (клик по отчёту, смена периода),
не при ресайзе, наведении и кликах внутри панели. Web Animations из JS (без `@keyframes`):
столбики растут от оси 480 мс волной (задержка 12 мс на колонку), линия рисуется 760 мс,
заливка под ней открывается слева направо (clipPath), точки и подписи проявляются,
полосы долей и корзин растут слева. `prefers-reduced-motion` — без анимации.

**Когорты**
- `border-spacing:2px`. Ячейка: padding 7 3, радиус 6.
- Столбик «Пришло»: справа отступ 14 до первой ячейки.
- Строка медианы в шапке есть всегда (в режиме «от таблицы» — пустая).

**Контролы**

**Единая высота контролов — 34 px** (эталон — поиск каталога): поиск, выпадашки
«Вид»/«Настройки», кнопки и иконки тулбара, подложка вкладок, сегмент периода и
переключатели шапки. Внутренние мелкие контролы (tiny-вкладки внутри графиков,
«Ещё N», пилюли, пагинатор) — своей высоты.

| Элемент | Высота | Прочее |
|---|---|---|
| Поиск, выпадашка, кнопка тулбара | 34 | padding 0 12, радиус 9 (поиск — пилюля 999) |
| Иконка-кнопка тулбара | 34 × 34 | копировать, «список на весь чарт» (нажата — фон `blue-bg`) |
| Вкладки | 28 в подложке 34 (tiny 22) | подложка padding 3 (tiny 2), gap 3, радиус 12/9; счётчик-бейдж высоту не меняет |
| Сегмент периода | 30 в подложке 34 | padding 0 12, радиус 7 внутри подложки 9 |
| Переключатель шапки | 34 | ползунок 26 × 15 |
| Каретка раскрытия группы | 28 × 28 | подложка при наведении; отдельная цель клика, промах не уходит в строку |
| Общая каретка | 28 × 28 | в строке «Итого по области», на вертикали строчных кареток: всё свёрнуто — ▸ раскрывает на уровень вглубь, что-то раскрыто — ▾ сворачивает всё (дерево УС-3…УС-7 целиком не раскрывается — тысячи строк) |
| Пилюля фильтра | 24 | padding 0 10, с × — справа 5; gap между пилюлями 6 |
| Кнопка «Ещё N ▾» | 26 | padding 0 10, радиус 8 |

«Кто смотрит»: полоса корзин → тулбар — 18 px, тулбар → таблица — 12 px. Выгрузка —
одна иконка «Копировать» (TSV в буфер, вставка в Excel раскладывает по колонкам);
рядом иконка «Список на весь чарт» — прячет KPI и «Что видно», корзины и настройки
остаются; повторный клик возвращает.

**Графики (SVG)**
- Зазор между графиками в стопке — 26 (`chart-gap`).
- Столбик: зазор 8, ширина 3…72 px. Воздух над марками под подписи ×1,3
  (плотная сетка ×1,22).
- Ось X: линия 1 px `rgb(155,164,181)` по ширине колонок, подписи 10,5.
  Начало месяца/года — 600 и `#3a3f4a`, вторая строка подписи — год/месяц.
- Подпись значения — у каждого столбика и у каждой точки линии, с белой обводкой
  3 px.

**«Что видно в данных»**
- Свёрнуто: строка 42 px (padding 0 8 0 14), иконка 20 × 20, первый факт с «…»,
  справа «Ещё N ▾».
- Раскрыто: список растёт вниз, отступ слева 44 (текст под текстом, не под
  иконкой), gap 8. У пункта точка важности 7 px, заголовок 500 и пояснение `muted`.

**Тултип**
- Радиус 9, padding 7 10, max-width 260, мягкая двойная тень.
- Заголовок: 10, КАПИТЕЛЬ. Строки: метка 11 `muted` → значение 12,5 / 500 справа.
  Подвал: 10,5 над линией `line2`.
- Появляется у курсора (+14, +18) и едет за ним. У края окна отражается.

### 16.6. Поведение, завязанное на вёрстку [ЯДРО профиля]

1. **Клик не двигает вёрстку.** Постоянную высоту держат строки фильтров, вкладки
   (с бейджем внутри), шапка когорт (резервная строка) и строка «Что видно»
   (свёрнутая). Раскрытие «Что видно» — единственное намеренное изменение
   высоты: графики под ним ужимаются.
2. **Пилюли фильтров.** Одна — во всю ширину, у длинного имени «…». Две — не шире
   50% каждая, обе с «…», справа «Снять все». Три и больше — одна пилюля
   «Применено N фильтров»: список во всплывашке, × снимает всё. В строке —
   только снимаемое здесь же; выбор соседнего чарта не дублируется.
3. **Тултип — только когда добавляет информацию.** У строки каталога тултип есть
   (просмотры на человека, дата создания). У строк таблиц «Кто смотрит» — нет:
   всё и так в колонках. Длинные URL в тултип не кладутся.
4. **Пустое — пусто.** Без выбора над панелью нет ни подписи, ни подсказки.
5. **Общие куски кода дословно одинаковы** в каталоге и панели: CSS «ТАБЛИЦЫ»,
   CSS «ВЫБРАННЫЕ ФИЛЬТРЫ» и функция `filterRowHtml`. Правка — в обоих файлах
   сразу, с проверкой совпадения.

### 16.7. Проверка перед сдачей

- [ ] Новые кегли и веса только из 16.2 и 16.3; 700 нет нигде.
- [ ] Литеральные цвета — только в блоке токенов и в рисовальном слое.
- [ ] Клик по строке, вкладке и режиму не сдвигает соседние блоки (замер
      `getBoundingClientRect` до и после).
- [ ] Крайние ступени легенды подсвечивают ячейки.
- [ ] Две длинные пилюли помещаются в строку, три сворачиваются в одну.
- [ ] Тултип едет за курсором и у правого/нижнего края не вылезает.
- [ ] Блоки «ТАБЛИЦЫ» и «ВЫБРАННЫЕ ФИЛЬТРЫ» совпадают в двух виджетах.

### 16.8. Отличия от ядра и почему

| Ядро (разд. 2–4) | Профиль Proteus Adoption | Почему |
|---|---|---|
| Веса 400 / 600 / 700 (+500) | 400 / 500 / 600, 700 нет | владелец: «не всё жирным», в плотном виджете 700 кричит; ступени сдвинуты на одну вниз |
| `--ink:#1f1f1f`, `--ink2:#3a3f4a` | `#23272e`, `#454b55` | мягче на белом, ближе к интерфейсу Proteus |
| Акцент `--act:#2b6cff` | кобальт `#245FD4` + `act-ink #1B4AA8`, `act-line #C3D4F5` | основной синий отчёта (решение владельца 2026-09-25, было `#0073A0`), совпадает с «новыми» на графике; белый текст на нём 5,7:1 |
| Фон `--bg:#f4f5f7` | `#f6f6f6` | равен фону борда Proteus, виджеты не выглядят заплатками |
| Карточки с тенью `--shadow` | без тени, радиус 12 | карточки лежат на сером холсте борда, тень там лишняя |
| Шрифт Inter по ссылке Google Fonts | стек без ссылки | iframe Proteus; внешние ресурсы не подключаем |
| Заголовок блока 16 / 700 | 14,5 / 600 | два уровня карточек в узкой колонке, 16 перетягивал внимание с KPI |
| Жёлтый только для AI-плашек | жёлтый — нижний край шкалы когорт | дивергентная шкала «ниже/выше медианы», не светофор; красный/зелёный там читались бы как оценка |
| Подсказки через механизм разд. 5, у цели | тот же механизм, но якорь — курсор | владелец: «как на диаграмме» |
