...
//
// ЗАШИТЫЙ БАГ: поиск фильтрует, но ПОЛНЫМ render() — DOM пересобирается на
// каждый символ вместе с самим полем, фокус падает на body, каретка
// пропадает, продолжить печатать нельзя (баг реальной сессии
// «иерархический-фильтр»). E23 обязан пройти (список меняется), E23b — упасть.
...
window.__pvtState[CFG.ns] = window.__pvtState[CFG.ns]
  || { view: 'a', pop: false, q: '', bound: false, docBound: false };
...
function buildCSS() {
  ...
  return '<style>'
    ...
    + P + '-pop{display:none;position:absolute;top:34px;left:8px;padding:8px;'
...
function buildHTML() {
  ...
  var shown = 0;
  for (i = 0; i < MODEL.rows.length && shown < 5; i++) {
    if (state.q && String(MODEL.rows[i].cat).indexOf(state.q) === -1) { continue; }
    shown++;
    h.push('<div class="' + P + '-row">' + esc(MODEL.rows[i].cat) + ': '
      + fmtInt(MODEL.rows[i].val) + '</div>');
  }
  if (!shown) { h.push('<div class="' + P + '-row">Не найдено</div>'); }
  h.push('</div></div>');
...
(function mount() {
  try {
    ...
    // Поиск фильтрует ПОЛНЫМ render(): разметка пересобирается вместе с полем,
    // фокус теряется — зашитый баг E23b (каретка жива только без пересборки).
    overlay.addEventListener('input', function (e) {
      var t = e.target;
      if (!t.className || t.className.indexOf(CFG.ns + '-search') === -1) { return; }
      state.q = t.value;
      render();
    });
    // document-слушатель вешается ОДИН РАЗ под флагом из state: document
    // перезапуск скрипта переживает, overlay — нет (RETRO 65, S21).
    if (!state.docBound) {
      state.docBound = true;
      document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && state.pop) { state.pop = false; render(); }
      });
    }
    render();
  } catch (err) {
