...
//
// РЕГРЕССИЯ ЛОЖНОЙ ТРЕВОГИ E6: тултип-«плашка» с БЕЛЫМ фоном на белом стенде.
// Прежний E6 снимал 8×8 из ЦЕНТРА — байты совпадали при реально нарисованной
// рамке, и проверку приходилось оспаривать (сессия «иерархический-фильтр»).
// Новый E6 сэмплирует центр + периметр bbox и обязан здесь PASS.
//
...
function buildCSS() {
  ...
  return '<style>'
    ...
    + P + '-tip{position:fixed;display:none;opacity:0;background:#fff;color:'
    + CFG.colors.ink + ';border:1px solid #999;'
...
(function mount() {
  try {
    ...
    if (!tip) {
      ...
      tip.style.cssText = 'position:fixed;display:none;opacity:0;background:#fff;color:'
        + CFG.colors.ink + ';border:1px solid #999;'
        + 'padding:6px 8px;font-family:' + CFG.fonts.family + ';z-index:99;';
