...
# После рефакторинга проверки уехали из validate.py в engine/checks_*:
# ссылка «RETRO N» живёт там — сканируем движок, а не CLI-обёртку.
CHECK_ID = r"'([A-Z]{1,2}\d+[a-z]?)'"
# Случаи selftest — тоже автозащита: регрессия связки (кэш, отчёт) не имеет
# своего кода проверки, но ловится именованным случаем.
CASE_ID = r'def (case_[a-z_]+)'
CHECKERS = [('engine/checks_code.py', CHECK_ID), ('engine/checks_process.py', CHECK_ID),
            ('smoke.mjs', CHECK_ID), ('check.py', CHECK_ID),
            ('engine/selftest_check.py', CASE_ID)]
