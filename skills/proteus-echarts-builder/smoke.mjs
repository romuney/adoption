...
// Стартовая папка поиска playwright (см. loadChromium). Для --env (ШАГ 0) чарта
// ещё нет — это рабочая папка проекта; в main() уточняется до папки чарта.
...
if (flags.has('--selftest')) {
  // fileURLToPath, а не .pathname: путь с кириллицей/пробелами приезжает
  // процент-закодированным, и fs.existsSync по нему молча не находит файлы.
  ...
  for (const [file, want] of Object.entries(exp)) {
    ...
    if (!/Итог:/.test(out)) {
      // Браузер не запустился (нет playwright / среда): это не «чекер сломан»,
      // а «вердикт неизвестен». Сравнивать не с чем — честно выходим с кодом 2.
      noBrowser = true;
      console.log(' -  ' + file.padEnd(24) + 'браузер не запустился — вердикт неизвестен');
      continue;
    }
    for (const m of out.matchAll(/^\s*[vX!?-]\s+(\S+)\s+(?:PASS|FAIL|WARN|N\/A|ОСПОР)\s/gm)) {
      exercised.add(m[1]);
    }
  ...
  // ── мета: согласованность с checks.json и покрытие фикстурами ────────────
  // Манифест — единый источник истины; E-коды обязаны совпадать с ним, а
  // каждая блокирующая проверка с fx:hard — печататься хоть одной фикстурой.
  // Без этого «проверка, которая ничего не проверяет, выглядит как прошедшая».
  console.log('\nМета: согласованность манифеста и покрытия');
  const manifestPath = path.join(here, 'checks.json');
  const manifest = fs.existsSync(manifestPath)
    ? JSON.parse(fs.readFileSync(manifestPath, 'utf8')).checks : {};
  const self = fs.readFileSync(fileURLToPath(import.meta.url), 'utf8');
  const literals = new Set();
  for (const m of self.matchAll(/['"]([A-Z]{1,2}\d+[a-z]?)['"]/g)) literals.add(m[1]);
  for (const m of self.matchAll(/['"]([A-Z]{2})['"]/g)) literals.add(m[1]); // префиксы ET/EM
  const mine = Object.entries(manifest)
    .filter(([id]) => id === 'B0' || /^E/.test(id));
  // Условные коды (cond) могут не печататься ни одним прогоном самотеста —
  // например, EM* живут только под --vs. Префиксные коды (ET2, EM3) собираются
  // в рантайме из 'ET'/'EM' + номер — литералом в исходнике их нет, и это
  // не признак пропажи: они объявлены, если объявлен префикс.
  const prefixDeclared = (id) => /^[A-Z]{2}\d/.test(id) && literals.has(id.slice(0, 2));
  const ghost = mine
    .filter(([id, m]) => !m.cond && !literals.has(id) && !exercised.has(id)
      && !prefixDeclared(id))
    .map(([id]) => id).sort();
  if (ghost.length) {
    bad++;
    console.log(' X  записи checks.json, которых нет в smoke.mjs и ни в одном'
      + ' прогоне: ' + ghost.join(', '));
  } else {
    console.log(' v  каждый браузерный код манифеста объявлен в smoke.mjs');
  }
  if (noBrowser) {
    console.log(' -  покрытие фикстурами НЕ проверено: браузер не запустился'
      + ' (мета требует живого прогона). Запусти selftest в окружении'
      + ' с playwright (код 2).');
    process.exit(2);
  }
  const noFx = mine
    .filter(([id, m]) => m.fx === 'hard' && m.sev === 'block' && !exercised.has(id))
    .map(([id]) => id).sort();
  if (noFx.length) {
    bad++;
    console.log(' X  проверки с fx:hard не печатает ни одна фикстура: ' + noFx.join(', ')
      + ' — добавь фикстуру, иначе ложную тревогу поймает пользователь');
  } else {
    console.log(' v  каждая обязательная (fx:hard) проверка печатается фикстурой');
  }
  console.log(bad === 0
...
projDir = path.dirname(chartPath);

// ── playwright ───────────────────────────────────────────────────────────────
// ЕДИНАЯ точка поиска playwright. Скилл предлагает ставить пакет командой из
// документации тремя способами (SKILL.md, --env): в ПРОЕКТ `npm i -D playwright`
// (node_modules рядом с чартом), ГЛОБАЛЬНО `npm i -g playwright`, или рядом со
// скриптом скилла. Раньше код искал только в папке скилла и в глобальном npm
// root — то есть НЕ находил самый частый вариант: playwright, поставленный
// в проект ровно той командой, что показана в справке, молча уходил в «не
// найден» (код 2, SESSION_STATUS). Это контракт, а не эвристика: порядок обхода
// = порядку команд установки. Резолюция — стандартный алгоритм Node
// (createRequire идёт вверх по node_modules), ничего не переизобретаем.
async function loadFromDir(startDir, mods) {
  const req = createRequire(path.join(startDir, '__smoke_resolve__.js'));
  for (const m of mods) {
    let root = null;
    try { root = pkgRootOf(req.resolve(m)); } catch { continue; }
    if (!root) continue;
    for (const rel of ['index.mjs', 'index.js']) {
      ...
      try {
        ...
        const api = ns.chromium || (ns.default && ns.default.chromium);
        if (api) return api;
      } catch { /* дальше */ }
    }
  }
...
// Корень пакета по пути его входа: создаём от index.js до каталога
// с package.json. Нужен, чтобы добраться до index.mjs — только ESM-вход
// даёт настоящие именованные экспорты (chromium); import() CJS-входа отдаёт
// namespace с одним `default`, и `ns.chromium` там пуст.
function pkgRootOf(entry) {
...
async function loadChromium() {
  const mods = ['playwright', 'playwright-core'];
  const seen = new Set();
  const dirs = [];
  // 1. проект: от папки чарта вверх по node_modules (`npm i -D playwright`).
  //    Для --env чарта ещё нет — берём рабочую папку (скрипты запускаются
  //    из проекта, SKILL.md).
  dirs.push(projDir);
  // 2. глобально: `npm root -g` указывает на сам каталог node_modules, поэтому
  //    стартуем с его родителя — тогда createRequire находит пакет обычным
  //    путём `<родитель>/node_modules/<имя>` (`npm i -g playwright`).
  try {
    const root = execSync('npm root -g',
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
    if (root) dirs.push(path.dirname(root));
  } catch { /* нет npm */ }
  // 3. сам скилл (установка рядом со скриптом).
  ...
  for (const d of dirs) {
    ...
    seen.add(key);
    const mod = await loadFromDir(key, mods);
    if (mod) return mod;
  }
  return null;
}
...
const PROBE = `(function () {
  ...
  // Панель ≠ контрол: носитель data-view/role=tabpanel, который сам не кнопка
  // и не помечен ролью/действием. Критерий тот же, что у panels(), — теперь
  // и при отборе ПЕРЕКЛЮЧАЛОК: раньше панель и соседний по родителю контрол
  // (например, кнопка «открыть поповер») склеивались в фейковую группу,
  // E15 падал на исправном виджете, а его клики по «группе» открывали
  // поповер раньше блока состояний (ловится фикстурами popover-*).
  function isPanel(el) {
    var tag = el.tagName.toLowerCase();
    if (tag === 'button' || tag === 'a' || tag === 'select' || tag === 'option') return false;
    if (el.getAttribute('role') === 'tab') return false;
    if (el.hasAttribute('data-action') || el.hasAttribute('aria-selected')) return false;
    return el.hasAttribute('data-view') || el.getAttribute('role') === 'tabpanel';
  }
  ...
  window.__smoke = {
    ...
    // Произвольный селектор по индексу, с прокруткой в поле зрения — для
    // входа в СОСТОЯНИЯ (поповер, поиск), чьи контролы не подходят под
    // контракт триггеров [data-*].
    nodeAt: function (rootSel, sel, i) {
      var root = rootSel ? document.querySelector(rootSel) : document;
      if (!root) return null;
      var el = root.querySelectorAll(sel)[i];
      return el ? atOf(el) : null;
    },
    // Фокус внутри виджета: после открытия поповера поиск и клавиатура
    // обязаны заработать без второго клика.
    focusedIn: function (rootSel) {
      var root = rootSel ? document.querySelector(rootSel) : document;
      var a = document.activeElement;
      return !!(a && a !== document.body && root && root.contains(a));
    },
    // Первое ВИДИМОЕ текстовое поле в виджете — кандидат на поиск.
    searchInputAt: function (rootSel) {
      var root = rootSel ? document.querySelector(rootSel) : document.body;
      if (!root) return null;
      var els = slice(root.querySelectorAll(
        'input[type="text"],input[type="search"],input:not([type])')).filter(vis);
      if (!els.length) return null;
      var r = els[0].getBoundingClientRect();
      return { cx: Math.round(r.left + r.width / 2), cy: Math.round(r.top + r.height / 2) };
    },
    focusedIsInput: function (rootSel) {
      var root = rootSel ? document.querySelector(rootSel) : document;
      var a = document.activeElement;
      return !!(a && a.tagName === 'INPUT' && root && root.contains(a));
    },
...
async function main() {
  ...
  try {
    ...
    // Патчей ПЯТЬ: центр + четыре середины сторон bbox. Прежняя версия снимала
    // только центр 8×8 — плашечный тултип с однотонной серединой на белом
    // стенде давал одинаковые байты при реально нарисованной рамке: чистый
    // ложный FAIL, который приходилось ОСПАРИВАТЬ (сессия «иерархический
    // фильтр»). Инвариант — «тултип КРАСИТ экран»: рамка отрисованной плашки
    // отличается от фона всегда, поэтому периметр bbox входит в выборку.
    const firstVisible = t.results.find((r) => r.tip);
    if (firstVisible) {
      const vp = await page.viewportSize();
      const HALF = 4, INSET = 2;
      const tp = firstVisible.tip;
      const pts = [
        [tp.cx, tp.cy],
        [Math.round(tp.x + tp.w / 2), Math.round(tp.y + INSET)],
        [Math.round(tp.x + tp.w / 2), Math.round(tp.y + tp.h - INSET)],
        [Math.round(tp.x + INSET), Math.round(tp.y + tp.h / 2)],
        [Math.round(tp.x + tp.w - INSET), Math.round(tp.y + tp.h / 2)],
      ];
      const clips = pts.map(([cx, cy]) => ({
        x: Math.min(Math.max(cx - HALF, 0), vp.width - 2 * HALF - 1),
        y: Math.min(Math.max(cy - HALF, 0), vp.height - 2 * HALF - 1),
        width: 2 * HALF, height: 2 * HALF }));
      await page.mouse.move(2, 2); await wait(250);
      const offs = [];
      for (const clip of clips) offs.push(await page.screenshot({ clip }));
      ...
      for (let i = 0; i < clips.length; i++) {
        if (!(await page.screenshot({ clip: clips[i] })).equals(offs[i])) painted++;
      }
      check('E6', painted > 0,
        'тултип реально отрисован на экране (' + painted + '/' + clips.length
          + ' патчей изменились)',
        'пиксели не изменились ни в одном из 5 патчей (центр + периметр bbox)'
          + ' — узел есть, а на экране его нет'
          + (t.visible > 0
            ? '. ET1–ET4 при этом зелёные: похоже на однотонный тултип'
              + ' на однотонном фоне. Посмотри скриншот (--shots); если тултип'
              + ' реально виден — оспорь, не подбирай код:'
              + " --accept 'E6=тултип виден на скриншоте, патчи легли на однотонные зоны'"
            : ''));
    ...
    // ── E20–E24: кастомные поповеры и поиск внутри них ──────────────────────
    // Дыра прежнего покрытия: E15 требует ГРУППУ переключалок с признаком
    // выбора, E17 — нативный <select>. Кастомный дропдаун (data-action="open"
    // → div-поповер), поиск внутри него, каретка и закрытие не проверялись
    // ВООБЩЕ — и баги этого класса (скролл-сброс, потеря фокуса, тултип поверх
    // списка) ловились только ручными playwright-скриптами сессии. Здесь дым
    // входит в СОСТОЯНИЯ: открыт / с вводом в поиск / после Esc — и гоняет
    // инварианты (экран, консоль, фокус) в каждом. Состояние неприменимо
    // (контролов нет) — N/A, как у остальных проверок.
    const OPENERS = '[data-action="open"],[data-action="toggle"],[aria-haspopup="true"]';
    const sigCount = (sig) => parseInt((/#n=(\d+)/.exec(sig || '') || [0, 0])[1], 10);
    const openerPos = await page.evaluate(
      ([s, sel]) => window.__smoke.nodeAt(s, sel, 0), [rootSel, OPENERS]);
    if (!openerPos) {
      skip('E20', 'контролов открытия поповера нет ([data-action=open|toggle] / [aria-haspopup])');
      skip('E21', 'поповер не открывался');
      skip('E22', 'поповер не открывался');
      skip('E23', 'поповер не открывался — поля поиска внутри не было');
      skip('E23b', 'поля поиска не было');
      skip('E24', 'поповер не открывался');
    } else {
      const sigIdle = await page.evaluate((s) => window.__smoke.screen(s), rootSel);
      const nIdle = sigCount(sigIdle);
      const errsOpen0 = errs.length;
      await page.mouse.click(openerPos.cx, openerPos.cy);
      await wait(300);
      const sigOpen = await page.evaluate((s) => window.__smoke.screen(s), rootSel);
      const nOpen = sigCount(sigOpen);
      check('E20', sigOpen !== sigIdle,
        'поповер открывается (видимый экран изменился)',
        'клик по «открыть» не изменил видимый экран — обработчик не навешен,'
          + ' открывает не то, что рисует, или поповер спрятан двумя свойствами,'
          + ' а снимается одно (RETRO 44)');

      check('E21', errs.length === errsOpen0,
        'консоль чиста в открытом состоянии',
        'после открытия поповера ошибок в консоли: ' + (errs.length - errsOpen0));

      const focused = await page.evaluate((s) => window.__smoke.focusedIn(s), rootSel);
      check('E22', focused, 'фокус после открытия внутри виджета',
        'после открытия фокус остался на body: поиск и клавиатура поповера не'
          + ' заработают без дополнительного клика. Открыватель должен'
          + ' фокусировать поповер или поле поиска (focus())', true);

      const inp = await page.evaluate((s) => window.__smoke.searchInputAt(s), rootSel);
      if (!inp) {
        skip('E23', 'видимого текстового поля в поповере нет');
        skip('E23b', 'поля поиска не было');
      } else {
        ...
        await wait(300);
        const sigTyped = await page.evaluate((s) => window.__smoke.screen(s), rootSel);
        const emptyState = /не найдено|ничего не найд|нет совпаден|no results/i.test(sigTyped);
        check('E23', sigTyped !== sigOpen || emptyState,
          'поиск фильтрует список (экран изменился или показан empty-state)',
          'ввод в поиск не изменил видимый экран и не показал empty-state —'
            + ' поиск не фильтрует: обработчик input не навешен или пишет мимо'
            + ' состояния, из которого строится список');
        const caret = await page.evaluate((s) => window.__smoke.focusedIsInput(s), rootSel);
        // RETRO 68: полный render() на каждую букву пересоздаёт само поле ввода —
        // фокус и каретка пропадают, продолжить печать нельзя.
        check('E23b', caret, 'каретка в поле поиска жива после фильтрации',
          'после ввода фокус ушёл с поля: DOM пересобирается на каждый символ —'
            + ' каретка пропала, продолжить печатать нельзя. Фильтруй точечной'
            + ' правкой списка (пересборка контейнера строк), а не полным'
            + ' render() с пересозданием поля');
      }

      // Закрытие: Esc → повторный клик по открывателю → клик мимо. Достаточно
      // любого. Критерий — экран вернулся к idle-составу (меньше видимых
      // элементов или сигнатура совпала), а не «что-то изменилось»: Esc мог
      // законно очистить поиск, не закрыв поповер.
      await page.keyboard.press('Escape');
      await wait(250);
      let sigNow = await page.evaluate((s) => window.__smoke.screen(s), rootSel);
      let closed = sigNow === sigIdle || sigCount(sigNow) <= nIdle;
      if (!closed) {
        await page.mouse.click(openerPos.cx, openerPos.cy);
        await wait(250);
        sigNow = await page.evaluate((s) => window.__smoke.screen(s), rootSel);
        closed = sigNow === sigIdle || sigCount(sigNow) <= nIdle;
      }
      if (!closed) {
      ...
      check('E24', closed,
        'поповер закрывается (Esc / повторный клик / клик мимо)',
        'поповер не закрылся ни одним из трёх способов — проверь руками;'
          + ' возможно, это задумано (модальный фильтр с явным «Применить»)', true);
      // В состояния входили и вышли: E7 меряет разметку от своей точки отсчёта.
    }
