{#- pa_aud v3 — панель вкладки «Аудитория» (чарт «Аудитория: ЦА области»).
    ...
    Слушает полоску (period_param, pub_f/act_f/exc_f) и СВОЙ эмит применённой ЦА (самовлияние ВКЛ):
      ...
      ca_head_f ('1' — руководители, 'n' — не руководители) · ca_adg_f (AD-группы прав: член хотя бы одной).
      Есть хоть одно — ЦА «ПО УСЛОВИЯМ»
    ...
    Ответ — ЛЮДИ области (проценты считает чарт):
      ...
      section = 'adg' — все AD-группы прав Proteus для настройки ЦА: k = группа, n = людей штата, a = 1 — даёт
                      доступ к области;
      section = 'total' — n = штат, k = названия до 3 отчётов области, state_j — эхо условий
                      (period, n, mode, sel, exc, caMode acc|cond, ca{org, spec, stream, hq, it, head, adg}, md, areaN). Нет строк 'n' при
...
UNION ALL
{# Все AD-группы прав Proteus (как фильтр «AD группа» старого борда) — список в настройке ЦА:
   n — людей штата в группе (pa_adg_size), a = 1 — группа даёт доступ к отчётам области. #}
