...
(function (global) {
  ...
  /* --------------------- Ось периодов «как календарь» ---------------------
     Верхняя строка — сам период (день, номер недели, месяц, квартал), нижняя —
     метка СМЕНЫ периода: у дней и недель это месяц, у месяцев и кварталов —
     год. В точке смены верхняя метка ЖИРНАЯ: смена читается без второго ряда.
     Метка всегда двухстрочная (пустая нижняя строка занимает место), поэтому
     базовые линии соседних меток не пляшут. Всё через rich-стили ECharts. */
  const AXIS_RICH = {
    a: { fontWeight: 400, color: C.axis, fontSize: 10.5, lineHeight: 13, align: 'center' },
    A: { fontWeight: 700, color: '#3a3f4a', fontSize: 10.5, lineHeight: 13, align: 'center' },
    b: { fontWeight: 600, color: '#8a909c', fontSize: 9.5, lineHeight: 12, align: 'center' },
  };
  function periodAxisLabels(buckets, grain) {
    let data;
    const main = (bold) => '{' + (bold ? 'A' : 'a') + '|';
    const sub = (t) => (t ? '{b|' + t + '}' : '');
    if (grain === 'd') {
      data = buckets.map((ts, i) => {
        const d = new Date(ts), p = i ? new Date(buckets[i - 1]) : null;
        const ch = d.getUTCDate() === 1 || (p && p.getUTCMonth() !== d.getUTCMonth());
        return main(ch) + d.getUTCDate() + '}\n' + (ch ? sub(U.MONTHS[d.getUTCMonth()]) : '');
      });
    } else if (grain === 'w') {
      data = buckets.map((ts, i) => {
        const d = new Date(ts), p = i ? new Date(buckets[i - 1]) : null;
        const ch = !!p && p.getUTCMonth() !== d.getUTCMonth();
        return main(ch) + 'W' + U.isoWeek(ts).week + '}\n' + (ch ? sub(U.MONTHS[d.getUTCMonth()]) : '');
      });
    } else if (grain === 'm') {
      data = buckets.map((ts, i) => {
        const d = new Date(ts), p = i ? new Date(buckets[i - 1]) : null;
        const ch = !!p && p.getUTCFullYear() !== d.getUTCFullYear();
        return main(ch) + U.MONTHS[d.getUTCMonth()] + '}\n' + (ch ? sub(String(d.getUTCFullYear())) : '');
      });
    } else {
      data = buckets.map((ts, i) => {
        const d = new Date(ts), p = i ? new Date(buckets[i - 1]) : null;
        const ch = !!p && p.getUTCFullYear() !== d.getUTCFullYear();
        return main(ch) + 'Q' + (Math.floor(d.getUTCMonth() / 3) + 1) + '}\n' + (ch ? sub(String(d.getUTCFullYear())) : '');
      });
    }
    return { data, rich: AXIS_RICH };
  }
  function periodAxis(gridIndex, buckets, grain) {
    const m = periodAxisLabels(buckets, grain);
    return {
      type: 'category', gridIndex, data: m.data,
      axisLine: { lineStyle: { color: C.axisLine } },
      axisTick: { show: false },
      axisLabel: {
        fontFamily: FONT, margin: 9, rich: m.rich,
        interval: 0,   // метка на КАЖДОЙ точке, без пропусков
      },
      boundaryGap: true,
    };
  }
  ...
  function dynamics(rows, grain, opt) {
    const o = opt || {};
    const off = o.hidden || {};          // гашение ступеней пилюльками легенды
    const n = rows.length;
    /* Итог считается ТОЛЬКО по оставшимся ступеням: погашенная пилюлька не
       прячет серию, а пересчитывает график — высота столбика, подпись итога,
       потолок оси и проценты в подсказке от того, что осталось. Иначе
       выключение ступени ничего не говорило бы о динамике. */
    const vis = rows.map((r) => (off.new ? 0 : r.new_users) +
      (off.react ? 0 : r.react_users) + (off.ret ? 0 : r.ret_users));
    ...
    /* Сегменты стека разделяет ступень светлоты, а не белая линия: обводка
       ...
       Погашенная пилюлькой серия остаётся в стеке нулями — высота соседей
       и подпись итога не сдвигаются. */
    const SHORT = { new_users: 'new', react_users: 'react', ret_users: 'ret' };
    const mk = (name, key, color) => ({
      name, type: 'bar', stack: 'u', xAxisIndex: 0, yAxisIndex: 0,
      barWidth: bw, barMaxWidth: BAR_MAX,
      itemStyle: { color },
      data: rows.map((r) => (off[SHORT[key]] ? 0 : r[key])),
    });
    ...
    /* Порядок в стеке снизу вверх: НОВЫЕ в основании, дальше вернувшиеся,
       ...
       Подпись итога и скругление — на САМОЙ ВЕРХНЕЙ видимой ступени. */
    const stack = [sNew, sRe, sRet].filter((s, i) => !off[['new', 'react', 'ret'][i]]);
    const top = stack[stack.length - 1] || sNew;
    top.label = valueLabel((p) => U.compact(vis[p.dataIndex]), n);
    /* Подписи чисел НЕ прячутся при столкновении, а расходятся по вертикали
       (moveOverlap: shiftY) — все числа над столбиками видны всегда. */
    top.labelLayout = { moveOverlap: 'shiftY' };
    top.itemStyle.borderRadius = [3, 3, 0, 0];
    ...
    return {
      ...
      grid: [
        { left: 6, right: 26, top: 10, bottom: '48%' },
        { left: 6, right: 26, top: '72%', bottom: 40 },
      ],
      /* Легенды-стека здесь больше нет: ступени переключаются HTML-
         пилюльками над графиком (клик гасит ступень, определения —
         в подсказках пилюлек), заголовок верхней панели — HTML-каптион
         в той же строке. В канве остаётся только заголовок нижней. */
      title: [
        { text: viewsTitle, left: 0, top: '64%', textStyle: Object.assign({}, TITLE_STYLE, { fontSize: 12 }) },
      ],
      ...
      tooltip: Object.assign({}, TOOLTIP_BASE, {
        ...
        formatter(ps) {
          ...
          const tot = vis[i] || 1;
          const pc = (v) => ' · ' + U.pct(v / tot * 100, 0);
          return head +
            tipRow(null, 'Всего на графике', U.nf(vis[i])) +
            (off.new ? '' : tipRow(C.new, newLabel, U.nf(r.new_users) + pc(r.new_users))) +
            (off.react ? '' : tipRow(C.react, 'Вернувшиеся', U.nf(r.react_users) + pc(r.react_users))) +
            (off.ret ? '' : tipRow(C.ret, 'Продолжающие', U.nf(r.ret_users) + pc(r.ret_users))) +
            tipEnd;
        },
      }),
      xAxis: [
        periodAxis(0, rows.map((r) => r.bucket), grain),
        periodAxis(1, rows.map((r) => r.bucket), grain),
      ],
      yAxis: [
        valAxis(0, { max: headroom(Math.max.apply(null, vis.concat([1])), n) }),
  ...
  function audienceTimeline(rows, grain, opt) {
    ...
    return {
      ...
      grid: [
        ...
        { left: 6, right: 30, top: '67%', bottom: 40 },
      ],
      legend: {
