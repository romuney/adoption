...
(function (global) {
  ...
  function barTable(o) {
    ...
    rows.forEach((r) => {
      ...
      const act = o.cutKey
        ? ' data-slice="' + esc(o.cutKey) + '" data-val="' + esc(r.key) + '"' + (o.extraAttr || '') +
  ...
  function observations(list, id, fresh) {
    /* Геометрия блока НЕ зависит от наличия фактов: «пусто» рисуется тем же
       каркасом (иконка + заголовок + строка-вывод + плейсхолдер тега), что и
       с фактами — иначе при кликах, меняющих выборку, блок «дышал» шириной
       и высотой, и экран прыгал. Меняется цвет и текст, не коробка. */
    const empty = !list.length;
    const top = empty ? { sev: 'none', lead: 'Отклонений выше порогов нет: показатели в пределах обычного разброса' } : list[0];
    return '<div class="obs sev-' + (top.sev || 'none') + (empty ? ' is-empty' : '') + '" data-obs="' + id + '">' +
      '<div class="obs-h" role="button" tabindex="0" aria-expanded="false" aria-controls="obsb-' + id + '"' +
        (empty ? ' aria-disabled="true"' : '') + '>' +
        '<span class="obs-ico" aria-hidden="true">' + (empty ? '✓' : '!') + '</span>' +
        '<span class="obs-t">Что видно в данных' +
          (fresh ? ' <span class="obs-fresh">· ' + esc(fresh) + '</span>' : '') + '</span>' +
        '<span class="obs-lead">' + top.lead + '</span>' +
        '<span class="obs-tag">' + (empty ? 'в норме' : 'подробнее') + '</span>' +
  ...
  /* ------------------------------ Панель ---------------------------------
     Шапка — ОДНА строка, как у каталога: слева заголовок с обрезаемым
     подзаголовком, справа контролы (поиск, вкладки). Ничего не уезжает
     и не делает шапку двухэтажной. */
  function panel(o) {
