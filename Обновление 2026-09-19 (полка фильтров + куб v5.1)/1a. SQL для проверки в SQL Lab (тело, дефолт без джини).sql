WITH
  ...
  attrs_f AS (SELECT * FROM attrs WHERE 1=1),
  evd AS (
    ...
    FROM prod_proteus.proteus_adoption WHERE dashboard_id != 13040 AND login != 'svc_mon_otpp' AND published = 1 AND actual_flg = 1 AND NOT has(owners_string, login) GROUP BY dashboard_id, login, toStartOfDay(log_dttm)
  ),
  b0 AS (
    SELECT e.dashboard_id AS did, e.ow AS ow, e.cn AS cn, a.k4 AS k4, a.k5 AS k5,
      e.login AS login,
      toInt64(dateDiff('day', toStartOfDay(e.dte), toStartOfDay((SELECT md FROM maxd)))) AS k,
  ...
  pa AS (
    SELECT kd, k0, login,
      ...
      arraySort(groupArrayIf((k, toInt64(v)), k < 30)) AS kvw
