{#- ЗАПАСНОЙ pa_people (файл 3b) — на дневном факте, БЕЗ pa_pair и pa_dash_bkt. Брать, пока
    GP-параграфы «PA · пары» и «PA · просмотры по периодам» не запущены; ответ тот же, что у
    файла 3 (сверено на стенде), но медленнее. После запуска ноды — вернуть файл 3.
...
WITH
  maxd AS (SELECT max(log_dttm) AS md, min(log_dttm) AS ds FROM prod_proteus.pa_evd_day),
