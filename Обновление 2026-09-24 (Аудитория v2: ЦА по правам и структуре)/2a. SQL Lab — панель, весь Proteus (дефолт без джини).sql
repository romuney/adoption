-- 1
-- Панель «Аудитория: ЦА области» — без выбора в каталоге (весь Proteus под опциями по умолчанию).
...
SELECT 'total', '', arrayStringConcat((SELECT groupArray(nm) FROM (SELECT toString(dashboard_nm) AS nm FROM area ORDER BY dashboard_id LIMIT 3)), '\n'), '',
  ...
  concat('{"period":"d", "n":30', ', "md":"', toString(toDate((SELECT md FROM maxd))), '", "areaN":', toString((SELECT count() FROM area)), '}')
